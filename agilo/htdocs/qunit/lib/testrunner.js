...
function process() {
	...
	while(config.queue.length && !config.blocking) {
		config.queue.shift()();
		// prevent slow script warnings in long test suites
		// do it only every tenth test though to minimize the performance hit this workaround has
		if (index > 10)
...
function runTest() {
	...
	synchronize(function() {
		$('<p id="testresult" class="result"/>').html(['Tests completed in ',
			...
			'<span class="bad">', config.stats.bad, '</span> asserts of <span class="all">', config.stats.all, '</span> failed in ', $('ol#tests > li').length, ' tests.']
			.join(''))
			.appendTo("body");
