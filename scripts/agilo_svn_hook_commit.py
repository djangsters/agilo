...
REPOS="$1"
REV="$2"
REPO_ARG="" # set this to "--repo=<repo_name>" if you use trac 0.12 multirepo support
TRAC_ENV='/var/lib/trac/test/'
/var/lib/svn/local/hooks/agilo_svn_hook_commit.py -s post -e "$TRAC_ENV" -r "$REV" "$REPO_ARG" || exit 1
[----------- cut here -------------]
"""
...
Use as a SVN hook, options:
     ...
     -R --repo=     <repository name> (only applicable for trac > 0.12)
'''
